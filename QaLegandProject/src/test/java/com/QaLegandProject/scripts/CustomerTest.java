...
public class CustomerTest extends TestBase{
 ...
 public void verifyingStatus()
 {
	...
	//objCustomer.statusUpdate(Constants.textStatus);
	//Assert.assertEquals(objCustomer.checkingStatus(), Constants.textStatus);
 }
@Test(priority=12,description="verifying picture is uploaded in the add reparation page",retryAnalyzer=com.QaLegandProject.Listeners.RetryAnalyzer.class,groups= {"smoke"})

 
 public void verifyingpictureUpload()
 {
	objCustomer=new Customerpage(driver);
	objCustomer.uploadFile(Constants.picpath);
 }
